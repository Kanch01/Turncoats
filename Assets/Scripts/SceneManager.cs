...
public class GameFlowManager : MonoBehaviour
{
    ...
    {
        // Assign roles
        State.player0Role = Role.Hero;
        State.player1Role = Role.Boss;

        // Assign control inputs
        if (UnityEngine.InputSystem.Gamepad.all.Count > 0)
            State.player0Device = UnityEngine.InputSystem.Gamepad.all[0];
        if (UnityEngine.InputSystem.Gamepad.all.Count > 1)
            State.player1Device = UnityEngine.InputSystem.Gamepad.all[1];
    ...
    private IEnumerator Start()
    {
        if (debugMode)
        {
            ...
            // Track currently loaded scene
            currentLoadedContentScene = SceneManager.GetActiveScene().name;

            // Apply camera & input for the active scene
            SetCameraForActiveScene();
            ApplyInputPolicyForScene(currentLoadedContentScene);

            yield break; // Skip main menu load
        }
